...
std::vector<BYTE> fetch_jpeg_image() {
    HINTERNET hInternet = InternetOpenA("HTTP", INTERNET_OPEN_TYPE_DIRECT, NULL, NULL, 0);
    HINTERNET hConnect = InternetOpenUrlA(hInternet, STREAM_URL, NULL, 0, INTERNET_FLAG_NO_CACHE_WRITE, 0);
...
HBITMAP CreateBitmapFromMemory(std::vector<BYTE>& buffer) {
    ...
    if (hGlobal) {
        ...
        if (pData) {
            ...
            if (SUCCEEDED(CreateStreamOnHGlobal(hGlobal, TRUE, &pStream))) {
                hBitmap = Gdiplus::Bitmap::FromStream(pStream)->GetHBITMAP(Color(255, 255, 255));
                pStream->Release();
            }
        }
        GlobalFree(hGlobal);
...
void stream_video(HWND hwnd) {
    while (is_streaming) {
        std::vector<BYTE> image_data = fetch_jpeg_image();
        if (!image_data.empty()) {
            HBITMAP hBitmap = CreateBitmapFromMemory(image_data);
            if (hBitmap) {
                HDC hdc = GetDC(hwnd);
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    sensor_labels[0] = CreateWindow("STATIC", "0", WS_CHILD | WS_VISIBLE | SS_CENTER, 20, 200, 60, 60, hwnd, NULL, hInstance, NULL);
    sensor_labels[1] = CreateWindow("STATIC", "1", WS_CHILD | WS_VISIBLE | SS_CENTER, 85, 200, 60, 60, hwnd, NULL, hInstance, NULL);
